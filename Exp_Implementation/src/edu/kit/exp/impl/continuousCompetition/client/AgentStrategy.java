...
public class AgentStrategy {
    ...
    void processMarketUpdate(ContinuousCompetitionParamObject marketUpdate) throws InterruptedException {
        ...
        if (agentCore.isTriopolyTreatment == false) {
            ...
            if (myRole == 0) {
                updateMatrix(myLastAction, marketUpdate.getaFirmA()*5*marketUpdate.getoFirmA()*profitcoefficient, marketUpdate);
                newAction=runEpisode(marketUpdate);
            } else if (myRole == 1) {
                updateMatrix(myLastAction, marketUpdate.getaFirmB()*5*marketUpdate.getoFirmB()*profitcoefficient, marketUpdate);
                newAction=runEpisode(marketUpdate);
            }
        ...
        log4j.info("Current Q-value: {}", q[state/5][myLastAction/5]);
    ...
    private int selectAction(int state){
        ...
        index = getMaxActionIndex(state)*5;
    ...
    void updateMatrix(int action, Double reward, ContinuousCompetitionParamObject marketUpdate) {
        ...
        if (marketUpdate.getCountId() > 1) {
            ...
            if (getState(marketUpdate) == actionsofotherfirm[marketUpdate.getCountId() - 1]) {
                q[state/5][getMaxActionIndex(state)] = (1 - parameter.alpha) * q[state/5][getMaxActionIndex(state)] +
                        parameter.alpha * (((getMaxActionIndex(state)*5) * (60 - 1.8 * (getMaxActionIndex(state)*5) +
                                1.2 * (state)) * 1.25) + parameter.delta * nextMaxQ);
            }
        }
